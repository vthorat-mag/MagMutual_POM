...
public class IntegrateRallyRestAPI {
...

	{
		...
		/*
		 * //Read Test Set - kept for Future extension QueryRequest testSetRequest = new
		 * QueryRequest("TestSet"); testSetRequest.setFetch(new Fetch("Name",
		 * "Subscription", "DisplayName", "SubscriptionAdmin"));
		 ...
		 * JsonElement testSetQueryElement = testSetQueryResults.get(0); JsonObject
		 * testSetQueryObject = testSetQueryElement.getAsJsonObject(); String testSetRef
		...
		if (testcaseRefID.equals("")) {
			...
			// {"TestCaseResult": {"Build": "A","Date": "2018-05-03T21:42:13.000Z","Notes":
			// "Logs indicate pass, however code still doesn't produce expected result.",
			// "TestCase":
			...
			if (!createResponse.wasSuccessful()) {
				 ...
				 * GetRequest(testCaseResultRef); getRequest.setFetch(new Fetch("Date",
				 * "Verdict")); GetResponse getResponse = oRestApi.get(getRequest); JsonObject
				 * obj = getResponse.getObject(); System.out.println(String.
	...
	// Get the user reference, separating this out in a method as it could be called
	// only once.
