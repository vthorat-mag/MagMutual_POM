...
public class FinancePage extends CommonAction {
	...
	public FinancePage selectLastAccountFromAccountList() throws Exception {
		 ...
		 * accountNumList.get(lastAccountFromList).getAttribute("innerHTML").
		 * trim()); //selectValue(driver,
		 * accountNumList.get(lastAccountFromList),"Account Number " +
		 * accountNumList.get(lastAccountFromList).getAttribute("innerHTML").
		 * trim());
		 */
	...
	public FinancePage maintainAccount() throws Exception {
		...
		// should be different and between 1 to 10.
	...
	public PolicyIndicationPage policyEndorsementWithDate(String PolicyNo, String nextDayOfDueDate) throws Exception {
		...
		// enterDataIn(driver, effectiveFromDate, "12/12/2017", "Effective
		// Date");
		...
		if (verifyAlertDisplay(driver) == true) {
			...
			Date d = c.getTime();
			nextDay = formatter.format(d);
			exlUtil.writeData("TC42246", "AlternateNextDate", nextDay, 1,
					System.getProperty("user.dir") + "\\src\\main\\resources\\Form_Data.xlsx");
			acceptAlert(driver);
			clearTextBox(driver, effectiveFromDate, "Effective Date");
	...
	public FinancePage cashEntry() throws Exception {
		...
		Thread.sleep(2000);
		// clickButton(driver,checkNoOnCashEntryPage,"Check No on cash entry
		// page");// check no element is clicked to enable
		...
		// enterTextIn(driver, amountOnCashEntryPage, financePageDTO.Amount,
		// "Cash Entry Page's Amount");
	...
	public FinancePage validateBatch() throws Exception {
		...
		ExtentReporter.logger.log(LogStatus.INFO, "Amount:" + financePageDTO.Amount + "");
		// enterTextIn(driver, batchAmount, financePageDTO.Amount, "batch
		// Amount");
	...
	// Poll transaction inquirey list.
	...
	public void donwloadFinalSheetBySearchingAccountNo() throws Exception {
		...
		// policySearch(driver, accountNumber, Policy_Search, searchBtn,
		// policyList);
		Thread.sleep(2000);
	...
	/*
	 * //this method will select cancel from action drop down. public
	 * FinancePage selectCancelFromPolicyActionDDL() throws Exception {
	 * RateApolicyPage rateapolicypage = new RateApolicyPage(driver);
	 * CommonUtilities commutil = new CommonUtilities(); //String policyNo =
	 * rateapolicypage.policyNo(); Thread.sleep(3000);
	 * selectDropdownByValue(driver, policyActionDDL,
	 * financePageDTO.policyAction, "Policy Action");
	 * switchToFrameUsingElement(driver,
	 * driver.findElement(By.xpath("//iframe[contains(@src,'policyNo=" +
	 * financePageDTO.policyNum + "')]"))); Thread.sleep(3000);
	 * enterTextIn(driver,
	 * cancelDateOnCancelPopUp,commutil.getSystemDatemm_dd_yyyy(),"Cancel Date"
	 * ); verifyValueFromField(driver, accountingDateOnCancelPopUp, "N",
	 * "iseditable", "Accounting Date"); selectDropdownByValue(driver,
	 * cancelTypeaccountingDateOnCancelPopUp, cancelTypeDDLValue,
	 * "Cancel Type"); selectDropdownByValue(driver, cancelReasonOnCancelPopUp,
	 * cancelReasonDDLValue, "Cancel Type"); selectDropdownByValue(driver,
	 * cancelMethodOnCancelPopUp, cancelMethodDLValue, "Cancel Type");
	 * enterTextIn(driver, cancelCommentsCancelPopUp,
	 * financePageDTO.cancelComment, "Comments"); clickButton(driver,
	 * cancelBtnOnCancelPopUp, "Cancel"); invisibilityOfLoader(driver);
	 * Thread.sleep(5000); switchToParentWindowfromframe(driver);
	 * switchToFrameUsingElement(driver,
	 * driver.findElement(By.xpath("//iframe[contains(@src,'policyNo=" +
	 * financePageDTO.policyNum + "')]"))); clickButton(driver, tailCloseButton,
	 * "Tail Coverage page's Close"); invisibilityOfLoader(driver);
	 * Thread.sleep(2000); switchToParentWindowfromframe(driver);
	 * switchToFrameUsingElement(driver,
	 * driver.findElement(By.xpath("//iframe[contains(@src,'policyNo=" +
	 * financePageDTO.policyNum + "')]"))); Thread.sleep(3000);
	 * ExtentReporter.logger.log(LogStatus.INFO,
	 ...
	 * invisibilityOfLoader(driver); Thread.sleep(3000);
	 * switchToParentWindowfromframe(driver); return new FinancePage(driver); }
	 */
	...
	public void cancelwindowHandle() throws Exception {
		...
		Thread.sleep(2000);
		// enterTextIn(driver,
		// cancelDateOnCancelPopUp,commutil.getSystemDatemm_dd_yyyy(),"Cancel
		// Date");
	...
	public void verifyTailCoverage() {
		try {
			if (tailCloseButton.isDisplayed() == true) {
				 ...
				 * By.xpath("//iframe[contains(@src,'policyNo=" +
				 * financePageDTO.policyNum + "')]"))) == true)
				 */
				 ...
				 * switchToParentWindowfromframe(driver);
				 * switchToFrameUsingElement(driver, driver
				 * .findElement(By.xpath("//iframe[contains(@src,'policyNo=" +
				 * financePageDTO.policyNum + "')]"))); Thread.sleep(3000);
				 * ExtentReporter.logger.log(LogStatus.INFO,
				 * "Click on Ok button to save policy in Work in Progress status[WIP]"
				 * ); clickButton(driver, okPolicySaveAsWIPPopup, "Ok");
	...
	public void verifyChangeremoveCoverageAlertdisplayes() throws Exception {
		if (getAlertText(driver).contains("reset the owner of limit sharing groups")) {
			...
			// WebElement sharedGrpDescription =
			// driver.findElement(By.xpath("//table[@id =
			// 'sharedGroupListGrid'//tr//td//div[text()='UMB Shared']"));
			// clickButton(driver, sharedGrpDescription, "Shared Group
			// Description[UMB Shared]");
			...
			// rateApolicyPageDTO.phase.get(j)+
			...
			// and text()='Yes']"));
			// clickButton(driver, coverageName_UMB_CGL_INS_N,
			// "coverageName_UMB_CGL_INS");
			 ...
			 * xpath("//div[@id ='CSHAREDTLCOVERAGESHORTDESC' and text()='"
			 * +financePageDTO.
